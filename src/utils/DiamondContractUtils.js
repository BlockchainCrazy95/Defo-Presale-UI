...
import DiamondABI from '../contracts/DiamondNode.json';
import { RPC_URL, DiamondContractAddress as contractAddress } from './constants';

dotenv.config();

const CHAIN_ID = 43113 //process.env.CHAIN_ID

console.log("DiamondUtils: CHAIN_ID = ", CHAIN_ID)

/// Read Functions for Smart Contract

export const getMaxTotalNodes = async () => {
    Contract.setProvider(RPC_URL[CHAIN_ID])
    const contract = new Contract(DiamondABI, contractAddress)
    try {
        const res = await contract.methods.maxTotalDiamondNodes().call()
        return {
            success: true,
            status: res
        }
...
export const getDiamondSaleStatus = async () => {
    Contract.setProvider(RPC_URL[CHAIN_ID])
    const contract = new Contract(DiamondABI, contractAddress)
    try {
...
export const getDiamondUserBalance = async ( account ) => {
    Contract.setProvider(RPC_URL[CHAIN_ID])
    const contract = new Contract(DiamondABI, contractAddress)
    try {
...
export const mintNode = async (library, account, amount = 1) => {
    ...
    if(library)
    {
        ...
        contract = await new web3.eth.Contract(DiamondABI, contractAddress)
    } else {
        Contract.setProvider(RPC_URL[CHAIN_ID])
        contract = new Contract(DiamondABI, contractAddress)
    }
